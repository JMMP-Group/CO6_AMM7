MODULE obs_read_pco2
   !!======================================================================
   !!                       ***  MODULE obs_read_pco2  ***
   !! Observation diagnostics: Read the along track pco2 data from
   !!                          GHRSST or any pco2 data from feedback files
   !!======================================================================

   !!----------------------------------------------------------------------
   !!   obs_rea_pco2 : Driver for reading pco2 data from the feedback
   !!----------------------------------------------------------------------
   ...
   USE obs_pco2_io            ! I/O for pco2 files
   USE iom                      ! I/O
   ...
   PUBLIC obs_rea_pco2      ! Read the pco2 observations from the point data
...
CONTAINS
   ...
   SUBROUTINE obs_rea_pco2( kformat, &
      &                    pco2data, knumfiles, cfilenames, &
      &                    kvars, kextr, kstp, ddobsini, ddobsend, &
      &                    ldignmis, ldmod )
      !!---------------------------------------------------------------------
      !!
      !!                   *** ROUTINE obs_rea_pco2 ***
      !!
      !! ** Purpose : Read from file the pco2 data
      !!
      ...
      INTEGER, INTENT(IN) :: kvars    ! Number of variables in pco2data
      INTEGER, INTENT(IN) :: kextr    ! Number of extra fields for each var in pco2data
      ...
      !! * Local declarations
      CHARACTER(LEN=14), PARAMETER :: cpname='obs_rea_pco2'
      INTEGER :: ji
      INTEGER :: jj
      INTEGER :: jk
      INTEGER :: iflag
      ...
      INTEGER, DIMENSION(knumfiles) :: &
         & irefdate
      INTEGER :: iobsmpp
      INTEGER, PARAMETER :: ipco2maxtype = 1024
      INTEGER, DIMENSION(0:ipco2maxtype) :: &
         & ityp, &
         & itypmpp
      ...
      pco2_files : DO jj = 1, inobf
         ...
         IF(lwp) THEN
            WRITE(numout,*)
            WRITE(numout,*) ' obs_rea_pco2 : Reading from file = ', &
         ...
         ELSE 
            ...
            ELSEIF ( kformat == 1) THEN
               CALL read_pco2( TRIM( cfilenames(jj) ), inpfiles(jj), &
               &                 numout, lwp, .TRUE. )
            ELSE
      ...
      CALL obs_surf_alloc( pco2data, iobs, & 
                           kvars, kextr, kstp, jpi, jpj )
      
      ! * Read obs/positions, QC, all variable and assign to pco2data
      ...
      DO jk = 1, iobstot
         ...
         IF ( ( inpfiles(jj)%ptim(ji) >  djulini(jj) ) .AND.  &
            ...
            IF ( ( inpfiles(jj)%ivlqc(1,ji,1) == 1 ) .OR. &
               ...
               ! pco2 time coordinates
               pco2data%nyea(iobs) = iyea
               pco2data%nmon(iobs) = imon
               pco2data%nday(iobs) = iday
               pco2data%nhou(iobs) = ihou
               pco2data%nmin(iobs) = imin
               
               ! pco2 space coordinates
               pco2data%rlam(iobs) = inpfiles(jj)%plam(ji)
               pco2data%rphi(iobs) = inpfiles(jj)%pphi(ji)

               ! Coordinate search parameters
               pco2data%mi  (iobs) = inpfiles(jj)%iobsi(ji,1)
               pco2data%mj  (iobs) = inpfiles(jj)%iobsj(ji,1)
...
901            IF ( ios /= 0 ) THEN
                  ...
                  itype = 0
               ENDIF
               pco2data%ntyp(iobs) = itype
               IF ( itype < ipco2maxtype + 1 ) THEN
                  ityp(itype+1) = ityp(itype+1) + 1
               ELSE
                  IF(lwp)WRITE(numout,*)'WARNING:Increase ipco2maxtype in ',&
                     &                  cpname
               ENDIF

               ! Bookkeeping data to match observations
               pco2data%nsidx(iobs) = iobs
               pco2data%nsfil(iobs) = iindx(jk)

               ! QC flags
               pco2data%nqc(iobs) = inpfiles(jj)%ivqc(ji,1)

               ! Observed value
               pco2data%robs(iobs,1) = inpfiles(jj)%pob(1,ji,1)


               ! Model and MDT is set to fbrmdi unless read from file
               IF ( ldmod ) THEN
                  pco2data%rmod(iobs,1) = inpfiles(jj)%padd(1,ji,1,1)
               ELSE
                  pco2data%rmod(iobs,1) = fbrmdi
               ENDIF
            ENDIF
         ENDIF
      ...
      IF (lwp) THEN
         ...
         WRITE(numout,*)
         WRITE(numout,'(1X,A)')'pco2 data types'
         WRITE(numout,'(1X,A)')'-----------------'
         DO jj = 1,8
      ...
      !-----------------------------------------------------------------------
      DEALLOCATE( ifileidx, ipco2idx, zdat )
   ...
   END SUBROUTINE obs_rea_pco2

END MODULE obs_read_pco2
