MODULE obs_read_spm
   !!======================================================================
   !!                       ***  MODULE obs_read_spm  ***
   !! Observation diagnostics: Read the along track spm data from
   !!                          GHRSST or any spm data from feedback files
   !!======================================================================

   !!----------------------------------------------------------------------
   !!   obs_rea_spm : Driver for reading spm data from the feedback
   !!----------------------------------------------------------------------
   ...
   USE obs_spm_io            ! I/O for spm files
   USE iom                      ! I/O
   ...
   PUBLIC obs_rea_spm      ! Read the spm observations from the point data
...
CONTAINS
   ...
   SUBROUTINE obs_rea_spm( kformat, &
      &                    spmdata, knumfiles, cfilenames, &
      ...
      !!                   *** ROUTINE obs_rea_spm ***
      !!
      !! ** Purpose : Read from file the spm data
      !!
      ...
      INTEGER, INTENT(IN) :: kvars    ! Number of variables in spmdata
      INTEGER, INTENT(IN) :: kextr    ! Number of extra fields for each var in spmdata
      ...
      !! * Local declarations
      CHARACTER(LEN=14), PARAMETER :: cpname='obs_rea_spm'
      INTEGER :: ji
      INTEGER :: jj
      INTEGER :: jk
      INTEGER :: iflag
      ...
      INTEGER, DIMENSION(knumfiles) :: &
         & irefdate
      INTEGER :: iobsmpp
      INTEGER, PARAMETER :: ispmmaxtype = 1024
      INTEGER, DIMENSION(0:ispmmaxtype) :: &
         & ityp, &
         & itypmpp
      ...
      spm_files : DO jj = 1, inobf
         ...
         IF(lwp) THEN
            WRITE(numout,*)
            WRITE(numout,*) ' obs_rea_spm : Reading from file = ', &
         ...
         ELSE 
            ...
            ELSEIF ( kformat == 1) THEN
               CALL read_spm( TRIM( cfilenames(jj) ), inpfiles(jj), &
               &                 numout, lwp, .TRUE. )
            ELSE
      ...
      CALL obs_surf_alloc( spmdata, iobs, & 
                           kvars, kextr, kstp, jpi, jpj )
      
      ! * Read obs/positions, QC, all variable and assign to spmdata
      ...
      DO jk = 1, iobstot
         ...
         IF ( ( inpfiles(jj)%ptim(ji) >  djulini(jj) ) .AND.  &
            ...
            IF ( ( inpfiles(jj)%ivlqc(1,ji,1) == 1 ) .OR. &
               ...
               ! spm time coordinates
               spmdata%nyea(iobs) = iyea
               spmdata%nmon(iobs) = imon
               spmdata%nday(iobs) = iday
               spmdata%nhou(iobs) = ihou
               spmdata%nmin(iobs) = imin
               
               ! spm space coordinates
               spmdata%rlam(iobs) = inpfiles(jj)%plam(ji)
               spmdata%rphi(iobs) = inpfiles(jj)%pphi(ji)

               ! Coordinate search parameters
               spmdata%mi  (iobs) = inpfiles(jj)%iobsi(ji,1)
               spmdata%mj  (iobs) = inpfiles(jj)%iobsj(ji,1)
...
901            IF ( ios /= 0 ) THEN
                  ...
                  itype = 0
               ENDIF
               spmdata%ntyp(iobs) = itype
               IF ( itype < ispmmaxtype + 1 ) THEN
                  ityp(itype+1) = ityp(itype+1) + 1
               ELSE
                  IF(lwp)WRITE(numout,*)'WARNING:Increase ispmmaxtype in ',&
                     &                  cpname
               ENDIF

               ! Bookkeeping data to match observations
               spmdata%nsidx(iobs) = iobs
               spmdata%nsfil(iobs) = iindx(jk)

               ! QC flags
               spmdata%nqc(iobs) = inpfiles(jj)%ivqc(ji,1)

               ! Observed value
               spmdata%robs(iobs,1) = inpfiles(jj)%pob(1,ji,1)


               ! Model and MDT is set to fbrmdi unless read from file
               IF ( ldmod ) THEN
                  spmdata%rmod(iobs,1) = inpfiles(jj)%padd(1,ji,1,1)
               ELSE
                  spmdata%rmod(iobs,1) = fbrmdi
               ENDIF
            ENDIF
         ENDIF
      ...
      IF (lwp) THEN
         ...
         WRITE(numout,*)
         WRITE(numout,'(1X,A)')'spm data types'
         WRITE(numout,'(1X,A)')'-----------------'
         DO jj = 1,8
      ...
      !-----------------------------------------------------------------------
      DEALLOCATE( ifileidx, ispmidx, zdat )
   ...
   END SUBROUTINE obs_rea_spm

END MODULE obs_read_spm
