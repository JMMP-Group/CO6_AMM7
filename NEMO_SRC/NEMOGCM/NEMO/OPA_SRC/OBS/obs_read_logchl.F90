MODULE obs_read_logchl
   !!======================================================================
   !!                       ***  MODULE obs_read_logchl  ***
   !! Observation diagnostics: Read the along track logchl data from
   !!                          GHRSST or any logchl data from feedback files
   !!======================================================================

   !!----------------------------------------------------------------------
   !!   obs_rea_logchl : Driver for reading logchl data from the feedback
   !!----------------------------------------------------------------------
   ...
   USE obs_logchl_io            ! I/O for logchl files
   USE iom                      ! I/O
   ...
   PUBLIC obs_rea_logchl      ! Read the logchl observations from the point data
...
CONTAINS
   ...
   SUBROUTINE obs_rea_logchl( kformat, &
      &                    logchldata, knumfiles, cfilenames, &
      &                    kvars, kextr, kstp, ddobsini, ddobsend, &
      &                    ldignmis, ldmod )
      !!---------------------------------------------------------------------
      !!
      !!                   *** ROUTINE obs_rea_logchl ***
      !!
      !! ** Purpose : Read from file the logchl data
      !!
      ...
      TYPE(obs_surf), INTENT(INOUT) :: &
         & logchldata     ! logchl data to be read
      ...
      INTEGER, INTENT(IN) :: kvars    ! Number of variables in logchldata
      INTEGER, INTENT(IN) :: kextr    ! Number of extra fields for each var in logchldata
      ...
      !! * Local declarations
      CHARACTER(LEN=14), PARAMETER :: cpname='obs_rea_logchl'
      INTEGER :: ji
      INTEGER :: jj
      INTEGER :: jk
      INTEGER :: iflag
      ...
      INTEGER, DIMENSION(knumfiles) :: &
         & irefdate
      INTEGER :: iobsmpp
      INTEGER, PARAMETER :: ilogchlmaxtype = 1024
      INTEGER, DIMENSION(0:ilogchlmaxtype) :: &
         & ityp, &
         & itypmpp
      ...
      logchl_files : DO jj = 1, inobf
         ...
         IF(lwp) THEN
            WRITE(numout,*)
            WRITE(numout,*) ' obs_rea_logchl : Reading from file = ', &
         ...
         ELSE 
            ...
            ELSEIF ( kformat == 1) THEN
               CALL read_logchl( TRIM( cfilenames(jj) ), inpfiles(jj), &
               &                 numout, lwp, .TRUE. )
            ELSE
      ...
      ALLOCATE( iindx(iobstot), ifileidx(iobstot), &
         &      ilogchlidx(iobstot), zdat(iobstot) )
      jk = 0
      DO jj = 1, inobf
         DO ji = 1, inpfiles(jj)%nobs
            IF ( ( inpfiles(jj)%ptim(ji) >  djulini(jj) ) .AND. &
      ...
      CALL obs_surf_alloc( logchldata, iobs, & 
                           kvars, kextr, kstp, jpi, jpj )
      
      ! * Read obs/positions, QC, all variable and assign to logchldata
      ...
      DO jk = 1, iobstot
         ...
         IF ( ( inpfiles(jj)%ptim(ji) >  djulini(jj) ) .AND.  &
            ...
            IF ( ( inpfiles(jj)%ivlqc(1,ji,1) == 1 ) .OR. &
               ...
               ! logchl time coordinates
               logchldata%nyea(iobs) = iyea
               logchldata%nmon(iobs) = imon
               logchldata%nday(iobs) = iday
               logchldata%nhou(iobs) = ihou
               logchldata%nmin(iobs) = imin
               
               ! logchl space coordinates
               logchldata%rlam(iobs) = inpfiles(jj)%plam(ji)
               logchldata%rphi(iobs) = inpfiles(jj)%pphi(ji)

               ! Coordinate search parameters
               logchldata%mi  (iobs) = inpfiles(jj)%iobsi(ji,1)
               logchldata%mj  (iobs) = inpfiles(jj)%iobsj(ji,1)
...
901            IF ( ios /= 0 ) THEN
                  ...
                  itype = 0
               ENDIF
               logchldata%ntyp(iobs) = itype
               IF ( itype < ilogchlmaxtype + 1 ) THEN
                  ityp(itype+1) = ityp(itype+1) + 1
               ELSE
                  IF(lwp)WRITE(numout,*)'WARNING:Increase ilogchlmaxtype in ',&
                     &                  cpname
               ENDIF

               ! Bookkeeping data to match observations
               logchldata%nsidx(iobs) = iobs
               logchldata%nsfil(iobs) = iindx(jk)

               ! QC flags
               logchldata%nqc(iobs) = inpfiles(jj)%ivqc(ji,1)

               ! Observed value
               logchldata%robs(iobs,1) = inpfiles(jj)%pob(1,ji,1)


               ! Model and MDT is set to fbrmdi unless read from file
               IF ( ldmod ) THEN
                  logchldata%rmod(iobs,1) = inpfiles(jj)%padd(1,ji,1,1)
               ELSE
                  logchldata%rmod(iobs,1) = fbrmdi
               ENDIF
            ENDIF
         ENDIF
      ...
      IF (lwp) THEN
         ...
         WRITE(numout,*)
         WRITE(numout,'(1X,A)')'logchl data types'
         WRITE(numout,'(1X,A)')'-----------------'
         DO jj = 1,8
      ...
      !-----------------------------------------------------------------------
      DEALLOCATE( ifileidx, ilogchlidx, zdat )
   ...
   END SUBROUTINE obs_rea_logchl

END MODULE obs_read_logchl
