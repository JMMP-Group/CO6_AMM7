MODULE zdfmxl
!
! with added write statements   RJR
!
   !!======================================================================
   !!                       ***  MODULE  zdfmxl  ***
...
CONTAINS
   ...
   SUBROUTINE zdf_mxl( kt )
      ...
      INTEGER  ::   xi, xj       ! point of interest RJR
      ...
      IF( kt == nit000 ) THEN
         ...
         IF(lwp) THEN
           xi = 16
           xj = 11
           WRITE(numout,*) ' zdf_mxl writing numbers for gridpoint ', xi, xj
         ENDIF
      ENDIF
      ...
      ENDIF
         IF(lwp .AND. kt==2) THEN
           WRITE(numout,*) ' hmld(i,j) = ', hmld(xi,xj)
           WRITE(numout,*) ' hmlp(i,j) = ', hmlp(xi,xj)
           WRITE(numout,*) ' imld(i,j) = ', imld(xi,xj)
           WRITE(numout,*) ' hmlpt(i,j) = ', hmlpt(xi,xj)
         ENDIF
   ...
   SUBROUTINE zdf_mxl_tref()
      ...
      IF( zdf_mxl_alloc() /= 0 )   CALL ctl_stop( 'STOP', 'zdf_mxl_tref : unable to allocate arrays' )
         ...
         IF(lwp) THEN
           xi = 16
           xj = 11
           WRITE(numout,*) ' zdf_mxl_tref writing numbers for gridpoint ', xi, xj
         ENDIF
...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzdf_karaml in configuration namelist', lwp )
      ...
      ENDIF
         ...
         IF(lwp) THEN
           xi = 16
           xj = 11
           WRITE(numout,*) ' zdf_mxl_kara writing numbers for gridpoint ', xi, xj
         ENDIF
      ...
      IF ( ln_kara ) THEN
         ...
         IF(lwp .AND. kt==2) THEN
           WRITE(numout,*) ' ppdT_crit = ', ppdT_crit
           WRITE(numout,*) ' fsdept(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') fsdept(xi,xj,:)
           WRITE(numout,*) ' fsdepw(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') fsdepw(xi,xj,:)
           WRITE(numout,*) ' fse3w(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') fse3w(xi,xj,:)
           WRITE(numout,*) ' tsn(xi,xj,:,1)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') tsn(xi,xj,:,1)
           WRITE(numout,*) ' tsn(xi,xj,:,2)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') tsn(xi,xj,:,2)
           WRITE(numout,*) ' rhop(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') rhop(xi,xj,:)
           WRITE(numout,*) ' ztsn_2d(xi,xj,1) = ', ztsn_2d(xi,xj,1)
           WRITE(numout,*) ' ztsn_2d(xi,xj,2) = ', ztsn_2d(xi,xj,2)
         ENDIF
         ...
         IF(lwp .AND. kt==2) THEN
           WRITE(numout,*) ' zRHO1(xi,xj), zRHO2(xi,xj), zdelta_T(xi,xj) = ', zRHO1(xi,xj), zRHO2(xi,xj), zdelta_T(xi,xj)
         ENDIF
         ...
         ENDIF 
         IF(lwp .AND. kt==2) THEN
           WRITE(numout,*) ' zT(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') zT(xi,xj,:)
         ENDIF
         ...
         END DO 
         IF(lwp .AND. kt==2) THEN
           WRITE(numout,*) ' zdTdz(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') zdTdz(xi,xj,:)
           WRITE(numout,*) ' zmoddT(xi,xj,:)'
           WRITE(numout,'(''[ '',52(F20.10,'', ''))') zmoddT(xi,xj,:)
         ENDIF
         ...
         ik_ref = -1
         DO jk = jpkm1, 2, -1 
            !CEODWHERE( fsdept(:,:,jk) > ppz_ref ) 
            WHERE( fsdept(:,:,jk)*tmask(:,:,jk) > ppz_ref ) 
               ik_ref(:,:) = jk - 1 
               zT_ref(:,:) = zT(:,:,jk-1) + &
         ...
         IF(lwp .AND. kt==2) THEN
           DO jk = jpkm1, 2, -1 
             IF( fsdept(xi,xj,jk)*tmask(xi,xj,jk) > ppz_ref ) THEN
               WRITE(numout,*) ' setting ik_ref = jk-1 = ', ik_ref(xi,xj)
               WRITE(numout,*) ' setting zT_ref = zT(xi,xj,jk-1) ', zT(xi,xj,jk-1)
               WRITE(numout,*) '                + (zdTdz(xi,xj,jk-1) ', zdTdz(xi,xj,jk-1)
               WRITE(numout,*) '                * (ppz_ref - fsdept(xi,xj,jk-1) ) ', ppz_ref, fsdept(xi,xj,jk-1)
               WRITE(numout,*) '                = ', zT_ref(xi,xj)
             ENDIF
           END DO
         ENDIF
!CEODWORKAROUND deal with case that can't find grid center deeper than reference

         IF(lwp .AND. kt==2) THEN
           IF( ik_ref(xi,xj) == -1 )  THEN
             WRITE(numout,*) ' setting ik_ref = mbathy(xi,xj) = ', mbathy(xi,xj)
             WRITE(numout,*) ' setting zT_ref = ', zT(xi,xj,mbathy(xi,xj))
           ENDIF
         ENDIF
         DO jj=1,nlcj
            DO ji=1,nlci
              IF( ik_ref(ji,jj) == -1  )  THEN
                   ik_ref(ji,jj) = mbathy(ji,jj)
                   zT_ref(ji,jj) = zT(ji,jj,mbathy(ji,jj))
              ENDIF 
            ENDDO 
         ENDDO
         IF(lwp .AND. kt==2) THEN
             WRITE(numout,*) ' final ik_ref = ', ik_ref(xi,xj)
             WRITE(numout,*) ' final zT_ref = ', zT_ref(xi,xj)
         ENDIF
!CEOD END WORK AROUND if cant find cell deeper than references
          
         IF ( ANY( ik_ref  > jpkm1 ) ) THEN
            CALL ctl_stop( "STOP", &
         ...
         ELSE
            ...
            ENDWHERE 
            IF(lwp .AND. kt==2) THEN
              IF( fsdept(xi,xj,1) > ppz_ref )  THEN
                WRITE(numout,*) ' setting zT_ref = ', zT(xi,xj,1)
                WRITE(numout,*) ' setting ik_ref = 1'
              ENDIF
            ENDIF
         ENDIF
            ...
            END DO 
            IF(lwp .AND. kt==2) THEN
                  DO jk = ik_ref(xi,xj), mbathy(xi,xj)-1 
                     IF( zmoddT(xi,xj,jk) > ( ppiso_frac * zdelta_T(xi,xj) ) ) THEN 
                       WRITE(numout,*) ' setting ik_iso(i,j) = ', jk
                       WRITE(numout,*) ' setting ll_found(i,j) = ', ll_found(xi,xj)
                     ENDIF 
                  END DO 
            ENDIF
            ...
            END DO 
            IF(lwp .AND. kt==2) THEN
               IF( ll_found(xi,xj) .and. tmask(xi,xj,1) == 1.0 ) THEN 
                    WRITE(numout,*) ' zdelta_T(xi,xj) = ', zdelta_T(xi,xj)
                    WRITE(numout,*) ' zdTdz(xi,xj,ik_iso(xi,xj)) = ', zdTdz(xi,xj,ik_iso(xi,xj))
                    WRITE(numout,*) ' setting hmld_kara(i,j) = ', hmld_kara(xi,xj)
               ENDIF
            ENDIF
            ...
            END DO 
            IF(lwp .AND. kt==2) THEN
                  DO jk = ik_ref(xi,xj)+1, mbathy(xi,xj) 
                     IF( ll_found(xi,xj) ) EXIT 
                     IF( ll_belowml(xi,xj,jk) ) THEN                
                        zT_b = zT_ref(xi,xj) + zdelta_T(xi,xj) * &
                        &      SIGN(1.0, zdTdz(xi,xj,jk-1) ) 
                        zdT  = zT_b - zT(xi,xj,jk-1)                                      
                        zdz  = zdT / zdTdz(xi,xj,jk-1)                                       
                WRITE(numout,*) ' zt_b, zdT, zdz = ', zt_b, zdT, zdz
                WRITE(numout,*) ' setting hmld_kara(i,j) = ', hmld_kara(xi,xj)
                        EXIT                                                   
                     ENDIF 
                  END DO 
        ...
        IF(lwp) WRITE(numout,*) 
         ...
         IF(lwp) WRITE(numout,*) ' MOD(',i_cnt_25h,',25)',MOD( i_cnt_25h, 25)
               IF( ( MOD( i_cnt_25h, 25) == 0 ) .AND.  ln_kara_write25h )  THEN
                  CALL iom_put( "kara25h"  , ( hmld_kara_25h / 25._wp ) )
                  !CEOD now Reset i_cnt_25h as we need the first value of the next day using this midnight also
                  i_cnt_25h =1
