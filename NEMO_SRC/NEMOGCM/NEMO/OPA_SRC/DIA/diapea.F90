...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nam_pea in configuration namelist', lwp )
   ...
   IF( ierr /= 0 )   CALL ctl_stop( 'dia_pea_init: failed to allocate tmp_pea_S_mean_rho array' )
    ...
    DO jj = 1,jpj
       DO ji = 1,jpi
          DO jk = 1,jpk
             IF ( isnan(tmp_pea_rho(ji,jj,jk)) ) tmp_pea_rho(ji,jj,jk)=0.
             IF ( isnan(tmp_pea_TS_mean_rho(ji,jj,jk)) ) tmp_pea_TS_mean_rho(ji,jj,jk)=0.
             IF ( isnan(tmp_pea_S_mean_rho(ji,jj,jk)) ) tmp_pea_S_mean_rho(ji,jj,jk)=0.
          ENDDO
       ENDDO
    ENDDO   
       ...
       DO ji = 1,jpi
          ...
          IF ( tmask(ji,jj,1) == 1.0_wp ) THEN ! for sea points
             ...
             !for each level...
                ...
                IF ( tmask(ji,jj,jk) == 1.0_wp ) THEN
                   maxz = maxz + (fse3t_n(ji,jj,jk)*wgt_co_mat(ji,jj,jk))
                ENDIF
             ENDDO
            
            
             !divide by the depth
             IF ( pea(ji,jj).ne.0 .and. maxz.ne.0 ) THEN
                pea(ji,jj) = pea(ji,jj)/maxz
             ENDIF
             IF ( peat(ji,jj).ne.0 .and. maxz.ne.0 ) THEN
                peat(ji,jj) = peat(ji,jj)/maxz
             ENDIF
             IF ( peat(ji,jj).ne.0 .and. pea(ji,jj).ne.0 ) THEN
    ...
    ENDDO
!    
    CALL iom_put( "pea" , pea*tmask(:,:,1) )                 ! pea
    CALL iom_put( "peat" , peat*tmask(:,:,1) )               ! pea
    CALL iom_put( "peas" , peas*tmask(:,:,1) )               ! pea
