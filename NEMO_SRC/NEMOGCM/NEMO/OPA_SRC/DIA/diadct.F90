...
#include "domzgr_substitute.h90"
  ...
  INTEGER, PARAMETER :: nb_point_max  = 150  ! maximum number of points in a single section
...
902  IF( ios /= 0 ) CALL ctl_nam ( ios , 'namdct in configuration namelist', lwp )
  ...
  SUBROUTINE dia_dct(kt)
     ...
     IF( MOD(kt,nn_dct)==0 .or. &                ! compute transport every nn_dct time steps
        ...
        DO jsec=1,nb_sec
           ...
           WRITE(numout,*) 'Calling transport with lldebug override', jsec
  ...
  SUBROUTINE transport(sec,ld_debug,jsec)
     ...
     IF(sec%nb_point .NE. 0)THEN   
        ...
        IF( ld_debug )write(numout,*)"jsec, sec%direction(1) ", jsec, sec%direction(1)
